...
FPS_23_98 = 24 * 1000/1001


def frame2second(frame, fps=FPS_23_98):
    return 1 / fps * frame


def second2frame(second, fps=FPS_23_98):
    return second * fps
...
def img2wav(img_filename, wave_length_seconds=5.0, sample_rate=48000):
    ...
    wav_length = round(sample_rate * wave_length_seconds)
    return interp1d(wav_data, wav_length, dtype=np.int16)


def wav2img(wav_filename, start_time=0.0, duration_seconds=5.0, out_width=2048, out_height=1024):
    sample_rate, wave_data = wavfile.read(wav_filename)
    wav_sample_start = round(start_time * sample_rate)
    wav_sample_end = round(wav_sample_start + duration_seconds * sample_rate)
...
# end to end example:
# img = wav2img('input.wav', out_width=1024, out_height=512)
# img.save('encoded_wav.png')
# wav = img2wav('encoded_wav.png')
